...
class Inception(nn.Module):
    ...
    def forward(self,x):
        ...
        result=self.activa(torch.cat((branch1[:,:,2:-1],branch2[:,:,1:-1],branch3,branch4[:,:,1:-1]),1))
        return result
class CNNTextInception_new(BasicModule):
    def __init__(self, opt ):
        super(CNNTextInception_new, self).__init__()
        self.model_name = 'CNNText_new'
        self.opt=opt
        ...
        if self.opt.type_=='char':
            ...
            self.fc = nn.Sequential(
                nn.Linear(13824*2,opt.linear_hidden_size),
            ...
            self.fc = nn.Sequential(
                nn.Linear(6656*2,opt.linear_hidden_size),
                nn.BatchNorm1d(opt.linear_hidden_size),
                nn.ReLU(inplace=True),
                nn.Linear(opt.linear_hidden_size,opt.num_classes)
            )
    ...
    def forward(self,title,content):
        ...
        title_out=self.title_conv(title.permute(0,2,1)).view(title.size(0),-1)
        content_out=self.content_conv(content.permute(0,2,1)).view(content.size(0),-1)
